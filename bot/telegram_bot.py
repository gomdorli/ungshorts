...
from keywords.keyword_fetcher import fetch_trending_keywords_from_daum
...
def handle_generate(update: Update, context: CallbackContext):
    ...
    if topic_input:
        ...
        if not keywords:
            update.message.reply_text("⚠️ Google Trends 실패 → daum 시도 중...")
            keywords = fetch_trending_keywords_from_daum()
        if not keywords:
